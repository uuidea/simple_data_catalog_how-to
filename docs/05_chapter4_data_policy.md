...
Beyond specifying what actions are permitted or prohibited, policies establish the roles and responsibilities that govern data relationships between publishers and users. ODRL policies define clear accountability structures by identifying the parties involved and their respective obligations, creating a foundation for trusted data sharing relationships.
...
In our implementation, ODRL policies are attached to datasets through the `hasPolicy` property, creating direct links between data resources and their governance frameworks. This integration enables automated policy enforcement systems to discover applicable policies simply by examining the catalog metadata. It also supports policy discovery for human users, who can understand the terms of use before accessing or downloading data.
...
The foundation of any policy system is the ability to grant permissions that enable data usage while establishing clear boundaries for acceptable use. The simplest scenario involves granting basic permissions for open data usage, where users can freely access, use, and share the data without restrictions. This approach maximizes data accessibility while providing clear legal terms through an appropriate license.
...
policies:
  - uid: "https://university.edu/policies/academic-use"
    ...
    permission:
      ...
      - action: "analyze"
        assignee: "Data Consumer"
        description: "Permission to analyze the data for research purposes"
    prohibition:
      - action: "commercialize"
        description: "Prohibition of commercial use of the data"
      - action: "sell"
        description: "Prohibition of selling the data or derived products"
```
...
policies:
  - uid: "https://dataspace.org/policies/reciprocal-sharing"
    ...
    permission:
      - action: "use"
        assignee: "Data Consumer"
        description: "Permission for commercial use of the data"
        duty:
          - action: "compensate"
      ...
      - action: "analyze"
        ...
        duty:
          - action: "shareImprovements"
            description: "Share derived data improvements with the provider"
    duty:
      ...
      - action: "reportViolations"
        assignee: "Data Consumer"
        description: "Report policy violations to the data provider"
```
...
The policy could be applied in a data marketplace scenario where organizations exchange valuable datasets under carefully managed terms. The duties ensure fair compensation and reciprocal benefit, while the prohibitions (which could be added separately) protect against misuse. This level of policy sophistication enables the creation of trusted data ecosystems where participants can share valuable assets with confidence that usage terms will be respected and enforced.
